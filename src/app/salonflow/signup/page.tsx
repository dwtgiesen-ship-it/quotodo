    ...
    <div className="grid min-h-full place-items-center bg-sf-bg2 px-4 py-10">
      <div className="w-full max-w-sm">
        ...
        <div className="rounded-2xl border border-sf-line bg-sf-card p-7 shadow-sm">
          <h1 className="font-heading text-[22px] font-semibold tracking-tight text-sf-ink">{t.signupTitle}</h1>
          <p className="mt-1 text-[14px] text-sf-ink2">{t.signupSubtitle}</p>
          <form onSubmit={submit} className="mt-6 space-y-3.5">
            <Field label={t.name} type="text" value={name} onChange={setName} autoFocus placeholder="Alex Rivera" />
            <Field label={t.email} type="email" value={email} onChange={setEmail} placeholder="you@salon.com" />
            <Field label={t.password} type="password" value={password} onChange={setPassword} placeholder={t.min8} />
            {error && <p className="rounded-lg bg-[#fdf0f2] px-3 py-2 text-[13px] text-[#d06277]">{error}</p>}
            <button disabled={busy || !email || password.length < 8} className="flex w-full items-center justify-center gap-1.5 rounded-xl bg-sf-navy py-3 text-[15px] font-semibold text-white transition-colors hover:opacity-90 disabled:opacity-50">
              {busy ? <Loader2 className="size-4 animate-spin" /> : <>{t.signupCta} <ArrowRight className="size-4" /></>}
            </button>
          </form>
          <div className="mt-3 flex items-center gap-1.5 text-[12px] text-sf-muted"><Check className="size-3.5 text-[#4F9A57]" /> {t.noCard}</div>
        </div>
        <p className="mt-4 text-center text-[14px] text-sf-ink2">{t.haveAccount} <Link href="/salonflow/login" className="font-semibold text-sf-ink hover:underline">{t.loginCta}</Link></p>
      </div>
    </div>
  );
}
