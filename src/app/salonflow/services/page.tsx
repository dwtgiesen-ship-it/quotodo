    ...
    <div className="mx-auto max-w-4xl px-6 py-6">
      ...
      <div className="overflow-hidden rounded-xl border border-[#e6e7e7] bg-white">
        {state.services.map((svc) => {
          ...
          return (
            <div key={svc.id} className="flex items-center gap-3 border-b border-[#f0f1f1] px-4 py-3 last:border-0">
              <span className="size-3 shrink-0 rounded-full" style={{ backgroundColor: style.accent }} />
              <div className="min-w-0 flex-1">
                <div className="text-[14px] font-medium text-[#2c2f2e]">{svc.name}</div>
                <div className="text-[12px] text-[#9fa5a4]">{svc.durationMin} min{svc.depositMinor ? ` · ${money(svc.depositMinor, state.settings.currency)} deposit` : ""}{svc.bookableOnline ? "" : " · in-salon only"}</div>
              </div>
              <span className="text-[14px] font-semibold text-[#2c2f2e]">{money(svc.priceMinor, state.settings.currency)}</span>
              <button onClick={() => setEditing(svc)} className="grid size-8 place-items-center rounded-md text-[#9fa5a4] hover:bg-[#f4f5f4]"><Pencil className="size-4" /></button>
      ...
      <div className="w-full max-w-md rounded-2xl bg-white p-5 shadow-xl" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="space-y-3">
          <label className="block">
            <span className="mb-1 block text-[12px] font-medium text-[#6b7280]">Name</span>
            <input value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} className="w-full rounded-lg border border-[#e6e7e7] px-3 py-2 text-[13px] outline-none focus:ring-2 focus:ring-[#dfe1e1]" />
          </label>
          <label className="block">
            <span className="mb-1 block text-[12px] font-medium text-[#6b7280]">Category</span>
            <select value={form.category} onChange={(e) => setForm({ ...form, category: e.target.value as Category })} className="w-full rounded-lg border border-[#e6e7e7] px-3 py-2 text-[13px] outline-none focus:ring-2 focus:ring-[#dfe1e1]">
          ...
          <label className="flex items-center gap-2 text-[13px] text-[#3f4544]">
            <input type="checkbox" checked={form.bookableOnline} onChange={(e) => setForm({ ...form, bookableOnline: e.target.checked })} />
            Available for online booking
          </label>
        </div>
        <button disabled={!form.name} onClick={() => onSave(form)} className="mt-5 w-full rounded-lg bg-[#1f2a4d] py-2.5 text-[14px] font-semibold text-white disabled:opacity-40">
          Save service
        </button>
      </div>
    </div>
...
function NumField({ label, value, onChange }: { label: string; value: number; onChange: (v: number) => void }) {
  return (
    <label className="block">
      ...
      <input type="number" value={value} onChange={(e) => onChange(Number(e.target.value))} className="w-full rounded-lg border border-[#e6e7e7] px-3 py-2 text-[13px] outline-none focus:ring-2 focus:ring-[#dfe1e1]" />
    </label>
  );
}
