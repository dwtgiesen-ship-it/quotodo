    ...
    <div className="mx-auto max-w-5xl px-6 py-6">
      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
        {state.membershipPlans.map((p) => {
          ...
          return (
            <div key={p.id} className="rounded-xl border border-[#e6e7e7] bg-white p-4">
              <div className="flex items-baseline justify-between">
                <h3 className="text-[16px] font-semibold">{p.name}</h3>
                <span className="text-[18px] font-semibold">{money(p.priceMinor, state.settings.currency)}<span className="text-[12px] font-normal text-[#9fa5a4]">/{p.interval}</span></span>
              </div>
              <div className="mt-3 space-y-1.5 text-[13px] text-[#3f4544]">
              ...
              <div className="mt-3 border-t border-[#f0f1f1] pt-2 text-[12px] text-[#9fa5a4]">{count} active member{count === 1 ? "" : "s"}</div>
            </div>
          );
        })}
      </div>
      ...
      <div className="overflow-hidden rounded-xl border border-[#e6e7e7] bg-white">
        {state.memberships.length === 0 && <p className="px-4 py-6 text-center text-[13px] text-[#9fa5a4]">No members yet.</p>}
        {state.memberships.map((m) => {
          const client = clientById(m.clientId);
          const plan = state.membershipPlans.find((p) => p.id === m.planId);
          return (
            <div key={m.id} className="flex items-center gap-3 border-b border-[#f0f1f1] px-4 py-3 last:border-0">
              ...
              <span className={`text-[12px] font-medium ${m.status === "active" ? "text-[#4F9A57]" : "text-[#9fa5a4]"}`}>{m.status}{m.renewsAt && m.status === "active" ? ` · renews ${m.renewsAt}` : ""}</span>
              {m.status === "active" && (
  ...
  return (
    <div className="fixed inset-0 z-50 grid place-items-center bg-black/30 p-4" onClick={onClose}>
      <div className="w-full max-w-sm rounded-2xl bg-white p-5 shadow-xl" onClick={(e) => e.stopPropagation()}>
        <div className="mb-4 flex items-center justify-between"><h3 className="text-[16px] font-semibold">Add member</h3><button onClick={onClose} className="text-[#9fa5a4]"><X className="size-4" /></button></div>
        <label className="mb-1.5 block text-[12px] font-medium text-[#6b7280]">Client</label>
        <select value={clientId} onChange={(e) => setClientId(e.target.value)} className="mb-3 w-full rounded-lg border border-[#e6e7e7] px-3 py-2 text-[13px]"><option value="">Select…</option>{clients.map((c) => <option key={c.id} value={c.id}>{c.name}</option>)}</select>
        <label className="mb-1.5 block text-[12px] font-medium text-[#6b7280]">Plan</label>
        <select value={planId} onChange={(e) => setPlanId(e.target.value)} className="mb-4 w-full rounded-lg border border-[#e6e7e7] px-3 py-2 text-[13px]">{plans.map((p) => <option key={p.id} value={p.id}>{p.name}</option>)}</select>
        <button disabled={!clientId || !planId} onClick={() => onAdd(planId, clientId)} className="w-full rounded-lg bg-[#1f2a4d] py-2.5 text-[14px] font-semibold text-white disabled:opacity-40">Subscribe</button>
      </div>
    </div>
  );
}
