    ...
    <div className="mx-auto max-w-lg px-4 py-8">
      <div className="rounded-2xl border border-sf-line bg-sf-card p-6 shadow-sm">
        <div className="mb-1 flex items-center justify-between">
          ...
          {step > 1 && step < 5 && (
            <button onClick={() => setStep((s) => (s - 1) as Step)} className="flex items-center gap-1 text-[13px] text-sf-ink2 hover:text-sf-ink">
        ...
        {step === 3 && (
          <Section title="Choose a day">
            <div className="grid grid-cols-4 gap-2">
              {DAYS.map((d, i) => (
                <button key={d.label} onClick={() => { setDay(i); setStart(null); setStep(4); }} className={`rounded-lg border px-2 py-3 text-center ${day === i ? "border-[#1f2a4d] bg-sf-soft" : "border-sf-line hover:bg-sf-soft"}`}>
                  <div className="text-[11px] uppercase text-sf-muted">{d.label}</div>
        ...
        {step === 4 && (
          <Section title="Choose a time">
            {slots.length === 0 ? (
              <p className="text-[13px] text-sf-muted">No free slots that day — try another day.</p>
            ) : (
              <div className="grid grid-cols-3 gap-2">
                {slots.map((t) => (
                  <button key={t} onClick={() => setStart(t)} className={`rounded-lg border px-2 py-2 text-[13px] ${start === t ? "border-[#1f2a4d] bg-sf-navy text-white" : "border-sf-line hover:bg-sf-soft"}`}>
            ...
            <div className="mt-4">
              <span className="mb-1.5 block text-[13px] font-medium text-sf-ink2">Who is this for?</span>
              <select value={clientId ?? ""} onChange={(e) => setClientId(e.target.value || null)} className="w-full rounded-lg border border-sf-line px-3 py-2 text-[13px] outline-none focus:ring-2 focus:ring-[#dfe1e1]">
                <option value="">Select a client…</option>
                {state.clients.map((c) => <option key={c.id} value={c.id}>{c.firstName} {c.lastName}</option>)}
              </select>
            </div>
            {service?.depositMinor && (
              <div className="mt-3 rounded-lg bg-sf-soft px-3 py-2.5 text-[13px] text-sf-ink2">
                A {money(service.depositMinor, state.settings.currency)} deposit secures your spot (demo — no real charge).
              </div>
            )}
            <button disabled={start == null || !clientId} onClick={submit} className="mt-4 w-full rounded-lg bg-sf-navy py-3 text-[14px] font-semibold text-white disabled:opacity-40">
              Confirm booking
            </button>
          </Section>
        )}
          ...
          <div className="py-6 text-center">
            {confirmed.ok ? (
              <>
                ...
                <p className="mt-1 text-[13px] text-sf-ink2">{service?.name} with {state.staff.find((s) => s.id === staffId)?.name} · {DAYS[day].label} {DAYS[day].date}, {start != null && fmtRange(start, start).split(" - ")[0]}</p>
                <p className="mt-1 text-[12px] text-sf-muted">A confirmation would be sent by SMS &amp; email. It now appears on the calendar.</p>
                <div className="mt-5 flex justify-center gap-2">
                  <Link href="/salonflow/calendar" className="rounded-lg bg-sf-navy px-4 py-2 text-[13px] font-medium text-white">View on calendar</Link>
                  <button onClick={() => { setStep(1); setServiceId(null); setStaffId(null); setStart(null); setClientId(null); setConfirmed(null); }} className="rounded-lg border border-sf-line px-4 py-2 text-[13px] font-medium">Book another</button>
                </div>
              </>
            ) : (
              <>
                <h2 className="text-[18px] font-semibold text-[#d06277]">Couldn't book</h2>
                <p className="mt-1 text-[13px] text-sf-ink2">{confirmed.reason}</p>
                <button onClick={() => setStep(4)} className="mt-4 rounded-lg bg-sf-navy px-4 py-2 text-[13px] font-medium text-white">Pick another time</button>
              </>
            )}
          </div>
        )}
      </div>
      <p className="mt-3 text-center text-[12px] text-sf-muted">Demo booking page · the same flow powers the future marketplace.</p>
    </div>
  );
}
    ...
    <div className="mb-5 mt-3 flex items-center gap-1.5">
      {labels.map((l, i) => (
        <div key={l} className="flex flex-1 items-center gap-1.5">
          <div className={`h-1 flex-1 rounded-full ${i < step ? "bg-sf-navy" : "bg-[#e6e7e7]"}`} />
        </div>
      ))}
    </div>
  );
...
function ServiceRow({ svc, selected, onClick, currency }: { svc: Service; selected: boolean; onClick: () => void; currency: string }) {
  ...
  return (
    <button onClick={onClick} className={`flex w-full items-center gap-3 rounded-lg border px-3 py-3 text-left ${selected ? "border-[#1f2a4d]" : "border-sf-line hover:bg-sf-soft"}`}>
      ...
      <div className="flex-1">
        ...
        <div className="text-[12px] text-sf-muted">{svc.durationMin} min</div>
      </div>
  ...
  return (
    <button onClick={onClick} className={`flex w-full items-center gap-3 rounded-lg border px-3 py-3 text-left ${selected ? "border-[#1f2a4d]" : "border-sf-line hover:bg-sf-soft"}`}>
      ...
      <div className="flex-1">
        ...
        <div className="text-[12px] text-sf-muted capitalize">{staff.role}</div>
      </div>
    </button>
  );
}
