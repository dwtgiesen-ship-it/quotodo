  ...
  return (
    <div className="mx-auto max-w-6xl px-6 py-6">
      <div className="mb-5 flex items-center justify-between">
        <div>
          <h1 className="font-heading text-[22px] font-semibold tracking-tight text-sf-ink">Dashboard</h1>
          <p className="text-[13px] text-sf-muted">{settings.name} · this week (Aug 13–19, 2025)</p>
        </div>
        <Link
          href="/salonflow/assistant"
          className="btn-accent flex items-center gap-1.5 rounded-full px-3.5 py-2 text-[13px] font-medium"
        >
      ...
      <div className="mt-4 grid grid-cols-1 gap-4 lg:grid-cols-2">
            ...
            {metrics.topServices.map(({ service, count }) => {
              ...
              return (
                <div key={service!.id} className="flex items-center gap-3">
                  <span className="w-32 shrink-0 truncate text-[13px] text-sf-ink">{service!.name}</span>
                  <div className="h-5 flex-1 overflow-hidden rounded bg-sf-soft">
                    <div className="h-full rounded" style={{ width: `${(count / maxSvc) * 100}%`, backgroundColor: style.accent }} />
                  </div>
                  <span className="w-6 text-right text-[12px] font-medium text-sf-ink2">{count}</span>
                </div>
              );
            })}
          </div>
          ...
          <table className="w-full text-[13px]">
            <tbody>
              {metrics.staffPerf.map(({ staff: m, count, rev }) => (
                <tr key={m.id} className="border-b border-sf-line last:border-0">
                  <td className="py-2">
                  ...
                  <td className="py-2 text-right font-medium text-sf-ink">{money(rev, settings.currency)}</td>
                </tr>
              ))}
            </tbody>
          </table>
        ...
        <Card title="Waitlist">
          {state.waitlist.length === 0 ? (
            <p className="pt-1 text-[13px] text-sf-muted">No one waiting. Cancellations will surface matches here.</p>
          ) : (
            <div className="space-y-2 pt-1">
              {state.waitlist.map((w) => {
                ...
                return (
                  <div key={w.id} className="flex items-center justify-between rounded-lg bg-sf-soft px-3 py-2">
                    <div className="flex items-center gap-2 text-[13px]">
                      <Clock className="size-4 text-sf-muted" />
                    ...
                    <button onClick={() => offerWaitlist(w.id, w.clientId, w.serviceId, w.windowDay)} className="rounded-md bg-sf-navy px-2.5 py-1 text-[12px] font-medium text-white hover:opacity-90">Book slot</button>
                  </div>
                );
              })}
            </div>
...
function Kpi({ label, value, trend, good }: { label: string; value: string; trend: string; good?: boolean }) {
  return (
    <div className="rounded-xl border border-sf-line bg-sf-card p-4">
      <div className="text-[12px] font-medium uppercase tracking-wide text-sf-muted">{label}</div>
      <div className="mt-1 text-[24px] font-semibold leading-tight text-sf-ink">{value}</div>
      <div className={`mt-1 flex items-center gap-1 text-[12px] ${good ? "text-[#4F9A57]" : "text-sf-ink2"}`}>
...
function Card({ title, children, accent }: { title: string; children: React.ReactNode; accent?: boolean }) {
  return (
    <div className={`rounded-xl border bg-sf-card p-4 ${accent ? "border-sf-accent/30" : "border-sf-line"}`}>
      <div className="mb-1 text-[13px] font-semibold text-sf-ink">{title}</div>
      {children}
    </div>
  );
}
...
function Insight({ text, action }: { text: string; action: string }) {
  return (
    <div className="flex items-start justify-between gap-3 rounded-lg bg-sf-soft p-3">
      <p className="text-[13px] leading-snug text-sf-ink2">{text}</p>
      <button className="flex shrink-0 items-center gap-1 rounded-md bg-sf-card px-2.5 py-1 text-[12px] font-medium text-sf-accent shadow-sm hover:bg-[#eef2fb]">
        {action}
