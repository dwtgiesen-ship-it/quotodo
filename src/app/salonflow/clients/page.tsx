    ...
    <div className="flex h-full">
      <div className="flex min-w-0 flex-1 flex-col bg-white">
        ...
        <div className="border-b border-[#eef0f2] px-5 py-3">
          <div className="relative max-w-md"><Search className="absolute left-3 top-1/2 size-4 -translate-y-1/2 text-[#9fa5a4]" /><input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search clients..." className="w-full rounded-lg bg-[#f4f5f4] py-2 pl-9 pr-3 text-[13px] outline-none placeholder:text-[#9fa5a4] focus:ring-2 focus:ring-[#dfe1e1]" /></div>
        </div>
        <div className="min-h-0 flex-1 overflow-auto">
          {filtered.map((c) => (
            <button key={c.id} onClick={() => setSelectedId(c.id)} className={`flex w-full items-center gap-3 border-b border-[#f0f1f1] px-5 py-3 text-left hover:bg-[#f8f9f9] ${selectedId === c.id ? "bg-[#f4f6fb]" : ""}`}>
              ...
              <div className="min-w-0 flex-1"><div className="truncate text-[14px] font-medium">{c.firstName} {c.lastName}</div><div className="truncate text-[12px] text-[#9fa5a4]">{c.email || c.phone}</div></div>
...
function ClientPanel({ client, serviceName }: { client: Client; serviceName: (id: string) => string }) {
  ...
  return (
    <aside className="hidden w-[380px] shrink-0 flex-col border-l border-[#e6e7e7] bg-white md:flex">
      <div className="flex items-center gap-3 px-5 py-4">
        ...
        <div className="min-w-0"><div className="text-[16px] font-semibold">{client.firstName} {client.lastName}</div><div className="text-[12px] text-[#9fa5a4]">Client since {new Date(client.since).toLocaleDateString("en", { month: "long", year: "numeric" })}</div></div>
      </div>
      ...
      <div className="min-h-0 flex-1 overflow-auto px-5 pb-4">
        ...
        <div className="mb-3 flex gap-2">
          <button onClick={() => { sendMessage(client.id, "sms", `Hi ${client.firstName}, a quick note from ${state.settings.name}.`); toast.success("Message sent (demo)"); }} className="flex flex-1 items-center justify-center gap-1.5 rounded-lg border border-[#e6e7e7] py-2 text-[12px] font-medium hover:bg-[#f8f9f9]"><Mail className="size-3.5" /> Message</button>
          <a href="/salonflow/calendar" className="flex flex-1 items-center justify-center gap-1.5 rounded-lg border border-[#e6e7e7] py-2 text-[12px] font-medium hover:bg-[#f8f9f9]"><Sparkles className="size-3.5" /> Book</a>
        </div>
        ...
        <Block title="Membership">
          {plan ? (
            ...
            <div className="flex flex-wrap gap-1.5">{state.membershipPlans.map((p) => <button key={p.id} onClick={() => { subscribe(p.id, client.id); toast.success(`Subscribed to ${p.name}`); }} className="rounded-full bg-[#f0f1f1] px-2.5 py-1 text-[12px] font-medium hover:bg-[#e6e7e7]">+ {p.name}</button>)}</div>
          ) : <p className="text-[12px] text-[#9fa5a4]">No membership.</p>}
        </Block>
        ...
        <Block title="Loyalty" action={can("manageClients") && (
          ...
          {ledger.length === 0 ? <p className="text-[12px] text-[#9fa5a4]">No activity.</p> : (
            <div className="space-y-1">{ledger.slice(0, 5).map((t) => <div key={t.id} className="flex items-center justify-between text-[12px]"><span className="text-[#6b7280]">{t.note || t.reason} · {t.createdAt}</span><span className={t.delta >= 0 ? "font-medium text-[#4F9A57]" : "font-medium text-[#d06277]"}>{t.delta >= 0 ? "+" : ""}{t.delta}</span></div>)}</div>
          )}
        </Block>
        ...
        {client.profileType !== "generic" && (
          <Block title={`${client.profileType === "pet" ? "Pet" : "Nail"} profile`}>
            <div className="space-y-1 text-[12px] text-[#3f4544]">{Object.entries(client.profileData).map(([k, v]) => <div key={k}><span className="text-[#9fa5a4] capitalize">{k}:</span> {Array.isArray(v) ? v.join(", ") : String(v)}</div>)}</div>
          </Block>
        )}

        {/* photos */}
        <Block title="Photos" action={can("manageClients") && <button onClick={() => { const s = SWATCHES[photos.length % SWATCHES.length]; addPhoto(client.id, swatch(s[0], s[1]), "reference"); toast.success("Photo added"); }} className="grid size-6 place-items-center rounded-md bg-[#f0f1f1]"><ImagePlus className="size-3.5" /></button>}>
          {photos.length === 0 ? <p className="text-[12px] text-[#9fa5a4]">No photos.</p> : (
        ...
        <Block title="Notes">
          {can("manageClients") ? (
            <textarea value={notes} onChange={(e) => setNotes(e.target.value)} onBlur={() => { if (notes !== client.notes) { updateClient(client.id, { notes }); toast.success("Notes saved"); } }} rows={2} placeholder="Add a note…" className="w-full resize-none rounded-lg border border-[#e6e7e7] px-3 py-2 text-[13px] outline-none focus:ring-2 focus:ring-[#dfe1e1]" />
          ) : <p className="text-[13px] text-[#3f4544]">{client.notes || "—"}</p>}
        </Block>

        {/* history */}
        <Block title="Appointment history">
          {history.length === 0 ? <p className="text-[12px] text-[#9fa5a4]">No appointments yet.</p> : (
            <div className="space-y-1.5">{history.map((a) => <div key={a.id} className="flex items-center justify-between rounded-lg bg-[#f8f9f9] px-3 py-2 text-[13px]"><span>{serviceName(a.serviceId)}</span><span className="text-[#9fa5a4] capitalize">{a.status.replace("_", " ")}</span></div>)}</div>
          )}
        </Block>
      </div>
    </aside>
...
function Stat({ label, value }: { label: string; value: string }) {
  return <div className="rounded-lg border border-[#eef0f2] p-2.5 text-center"><div className="text-[15px] font-semibold">{value}</div><div className="text-[11px] text-[#9fa5a4]">{label}</div></div>;
}
function Block({ title, children, action }: { title: string; children: React.ReactNode; action?: React.ReactNode }) {
  return <div className="mb-3 border-t border-[#eef0f2] pt-3"><div className="mb-1.5 flex items-center justify-between"><span className="text-[11px] font-semibold uppercase tracking-wide text-[#9fa5a4]">{title}</span>{action}</div>{children}</div>;
}
  ...
  return (
    <div className="fixed inset-0 z-50 grid place-items-center bg-black/30 p-4" onClick={onClose}>
      <div className="w-full max-w-md rounded-2xl bg-white p-5 shadow-xl" onClick={(e) => e.stopPropagation()}>
        <div className="mb-4 flex items-center justify-between"><h3 className="text-[16px] font-semibold">Add client</h3><button onClick={onClose} className="grid size-7 place-items-center rounded-md text-[#9fa5a4] hover:bg-[#f4f5f4]"><X className="size-4" /></button></div>
        ...
        <button disabled={!form.firstName} onClick={() => { const c = addClient(form); toast.success("Client added"); onCreated(c.id); }} className="mt-5 w-full rounded-lg bg-[#1f2a4d] py-2.5 text-[14px] font-semibold text-white disabled:opacity-40">Create client</button>
      </div>
    </div>
  );
}
function Field({ label, value, onChange }: { label: string; value: string; onChange: (v: string) => void }) {
  return <label className="block"><span className="mb-1 block text-[12px] font-medium text-[#6b7280]">{label}</span><input value={value} onChange={(e) => onChange(e.target.value)} className="w-full rounded-lg border border-[#e6e7e7] px-3 py-2 text-[13px] outline-none focus:ring-2 focus:ring-[#dfe1e1]" /></label>;
}
