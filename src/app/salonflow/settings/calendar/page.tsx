...
const STATUS_STYLE: Record<string, string> = {
  ...
  disconnected: "bg-sf-soft text-sf-muted",
};
  ...
  return (
    <div className="mx-auto max-w-4xl px-6 py-6">
      <div className="mb-1 flex items-center gap-2">
        <Link href="/salonflow/settings" className="grid size-7 place-items-center rounded-md text-sf-muted hover:bg-sf-card"><ChevronLeft className="size-4" /></Link>
        <h1 className="font-heading text-[22px] font-semibold tracking-tight">Calendar sync</h1>
      </div>
      <p className="mb-5 ml-9 text-[13px] text-sf-muted">Two-way, real-time sync with Google, Microsoft 365 &amp; Apple. Personal events block bookings; Schedulemode appointments mirror to staff calendars.</p>
      ...
      <div className="mb-4 grid grid-cols-2 gap-3 sm:grid-cols-4">
        ...
        <button onClick={run} disabled={busy} className="flex items-center justify-center gap-1.5 rounded-xl border border-sf-line bg-sf-card p-3 text-[13px] font-medium hover:bg-sf-soft disabled:opacity-50"><RefreshCw className={`size-4 ${busy ? "animate-spin" : ""}`} /> Run sync now</button>
      </div>
      ...
      <div className="space-y-2">
        {state.staff.map((m) => {
          ...
          return (
            <div key={m.id} className="rounded-xl border border-sf-line bg-sf-card p-4">
              ...
              <div className="flex flex-wrap items-center gap-2">
                {conns.map((c) => (
                  <div key={c.id} className="flex items-center gap-2 rounded-lg border border-sf-line py-1.5 pl-3 pr-1.5 text-[13px]">
                    ...
                    {c.lastSyncedAt && <span className="text-[11px] text-sf-muted">· synced {new Date(c.lastSyncedAt).toLocaleTimeString()}</span>}
                    ...
                    <button onClick={() => disconnect(c.id)} className="grid size-6 place-items-center rounded-md text-sf-muted hover:bg-[#fdf0f2] hover:text-[#d06277]"><Unplug className="size-3.5" /></button>
                  </div>
                ))}
                {PROVIDERS.filter((p) => !connectedProviders.has(p.id)).map((p) => (
                  <button key={p.id} onClick={() => connect(m.id, p.id)} disabled={busy} className="flex items-center gap-1.5 rounded-lg border border-dashed border-[#d6d8d8] px-3 py-1.5 text-[13px] text-sf-ink2 hover:bg-sf-soft disabled:opacity-50">
      ...
      <div className="overflow-hidden rounded-xl border border-sf-line bg-sf-card">
        {(status?.logs.length ?? 0) === 0 && <p className="px-4 py-6 text-center text-[13px] text-sf-muted">No sync activity yet. Connect a calendar to begin.</p>}
        {status?.logs.map((l) => (
          <div key={l.id} className="flex items-center gap-3 border-b border-sf-line px-4 py-2 text-[13px] last:border-0">
            ...
            <span className="flex-1 truncate text-sf-ink2">{l.message}</span>
            <span className="shrink-0 text-[11px] text-sf-muted">{new Date(l.at).toLocaleTimeString()}</span>
          </div>
        ))}
      </div>

      <p className="mt-4 text-center text-[12px] text-sf-muted">Live providers activate when their OAuth credentials are set (Google/Microsoft) or app-specific password (Apple). Until then connections run in demo mode through the same engine. See docs/salonflow/08-calendar-sync.md.</p>
    </div>
  );
}
...
function Stat({ icon: Icon, label, value, sub }: { icon: typeof Plug; label: string; value: string; sub?: string }) {
  return (
    <div className="rounded-xl border border-sf-line bg-sf-card p-3">
      <div className="flex items-center gap-1.5 text-[11px] font-medium uppercase tracking-wide text-sf-muted"><Icon className="size-3.5" /> {label}</div>
      <div className="mt-1 text-[18px] font-semibold leading-tight">{value}</div>
      {sub && <div className="text-[11px] text-sf-muted">{sub}</div>}
    </div>
  );
}
