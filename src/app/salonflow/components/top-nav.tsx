    ...
    <header className="flex h-14 shrink-0 items-center justify-between bg-[#1f2a4d] px-4 text-white/90">
      <div className="flex min-w-0 items-center gap-1">
        <Link href="/salonflow/dashboard" className="mr-2 flex items-center gap-2">
          ...
          <span className="hidden text-[15px] font-semibold text-white lg:inline">SalonFlow</span>
        </Link>
      ...
      <div className="flex shrink-0 items-center gap-1.5">
        ...
        {/* Role switcher — demonstrates the production permission matrix (docs/salonflow/04) */}
        <select
          value={role}
        ...
        <div className="ml-1 hidden items-center gap-2 md:flex">
          <div className="grid size-8 place-items-center rounded-full bg-[#5b6bb0] text-xs font-semibold text-white">AP</div>
          <div className="leading-tight">
            <div className="text-[13px] font-semibold text-white">Andre Pearl</div>
            <div className="text-[11px] capitalize text-white/60">{role} · {state.settings.name}</div>
          </div>
        </div>
      </div>
    </header>
